...
\author{1FCENdoge\footnotemark[1] \and Marc-Antoine\footnotemark[1] \and SYSU-SCC\footnotemark[3] \and Zeping Lee\footnotemark[2]}
\renewcommand{\thefootnote}{\fnsymbol{footnote}}
\footnotetext[1]{中山大学中法核工程与技术学院}
\footnotetext[2]{中国科学技术大学数学科学学院}
\footnotetext[3]{中山大学超算队}
\date{\today}
\maketitle
...
\begin{latex}
  \documentclass[degree-type=professional]{sysuthesis}
\end{latex}

\DescribeOption{language}
论文全文的主要语言。可选：\opt{chinese}（默认），\opt{english}。
\begin{latex}
  \documentclass[language=english]{sysuthesis}
\end{latex}
...
\end{itemize}
\begin{latex}
  \documentclass[output=electronic]{sysuthesis}
\end{latex}
...
\end{itemize}
\begin{latex}
  \documentclass[section-style=arabic]{sysuthesis}
\end{latex}
...
封面由 \cs{maketitle} 命令生成，其中的各项信息使用 \cs{sysusetup} 命令以
key-value 的方式填写，如：
\begin{latex}
  \sysusetup{
    title  = {论文中文题目},
...
有几点说明：
\begin{itemize}
  \item \cs{sysusetup} 使用 \pkg{kvsetkeys} 机制，配置项之间不能有空行，否则会报错。
...
对于特殊的章节，\cls{sysuthesis} 还提供了相应的环境：
\begin{itemize}
  \item 中文摘要：\env{abstract}
  \item 英文摘要：\env{abstract*}
  \item 第三种语言摘要（如果需要）：\env{abstract**}
...
\DescribeOption{keywords*}
\DescribeOption{keywords**}
摘要的关键词应使用 \cs{sysusetup} 的接口进行设置，在录入时使用西文逗号隔开，为满足中法核的需要，本科论文加入了法语摘要，
比如：
\begin{latex}
  \begin{abstract}
    这里是摘要。
